...
lines = [
    ...
    "",
    f"**當前已歸檔食譜總數**：`{len(recipes)}` 道精選素食料理（康寶官方：`{len(knorr_recipes)}` 道 ｜ 觀音山蔬食館：`{len(gys_recipes)}` 道）",
    "",
    "---",
    "",
    f"## 📚 1. 觀音山 · 素食料理簡單做 — 主食與精選系列 ({len(gys_recipes)} 道)",
    "",
...
lines.extend([
    ...
    f"├── www.knorr.com/          # 康寶官方食譜庫 ({len(knorr_recipes)} 道獨立資料夾與高清圖檔)",
    f"└── vegan.gys.org.tw/       # 觀音山蔬食館食譜庫 ({len(gys_recipes)} 道獨立資料夾與高清圖檔)",
    "```",
    "",
    "---",
...
print(f"Master README.md regenerated successfully with {len(recipes)} recipes!")
